...
The DB_* and POSTGRES_* variables use the same username and password so that the backend can successfully connect and authenticate with the database.
...
The quota shows 3 pods using 300m CPU and 384Mi memory in requests, that's exactly 3 × the LimitRange's defaults (100m/128Mi each), so I can see the LimitRange is actually being applied to every container automatically, not just sitting there unused.

I set pods: 6 so a rolling update (old pod + new pod running briefly) won't hit the limit. The per-container defaults are small on purpose this app is just a simple CRUD demo, not something that needs heavy resources, and my cluster only has 3 nodes to work with.
